...
def bm25(word,lexicon,doc,idf,doc_num,words):
...
def main():
    ...
    parser.add_argument('retrieval_mode', type=str, help='It only has three models(cosine, bm25)')
                ...
                if not line.isspace():
                    ...
                    if re.match(r'<title>',line):
                        ...
                        if len(score)==0:
                            continue
                        if args.expansion_mode=='feedback':
                            f_terms=feedback(int(ex_doc_nums),int(ex_term_nums),single_lexicon,score,doc_dict,idf_lexicon,word)
                        elif args.expansion_mode=='occurrence':
                            f_terms=co_occur(int(ex_term_nums),word,single_lexicon,single_dict,doc_dict)
                ...
                if not line.isspace():
                    ...
                    if re.match(r'<title>', line):
                        ...
                        if len(score) == 0:
                            continue
                        if args.expansion_mode=='feedback':
                            f_terms = feedback(int(ex_doc_nums),int(ex_term_nums), stem_lexicon,score, doc_dict, idf_lexicon,word)
                        elif args.expansion_mode == 'occurrence':
                            f_terms = co_occur(int(ex_term_nums), word, stem_lexicon, stem_dict, doc_dict)
                        for w in f_terms:
                            if w in stop_word_list or w  in r_list or w.isspace():
                                continue
                            word=word_tokenize_common(w)

...
def reduction(words,idf,lexicon,dic,word,thres,mode):
    temp = {}
    for i in words:
        index = lexicon[i]
        if index not in temp:
            ...
            if mode=='tfidf':
                temp[index] = idf[index] * max_tf
            elif mode=='query':
                temp[index] = words.count(i)
            else:
                print('No this sorting mode')
...
def main():
    ...
    parser.add_argument('sort_mode', type=str, help='It only has two modes(tfidf or query)')
            ...
            for line in generate_file_lines(f):
                if not line.isspace():
                    ...
                    if re.match(r'</top>', line):
                        word=list(set(words))
                        content = False
                        q_terms=reduction(words,idf_lexicon,single_lexicon,single_dict,word,float(threshold),args.sort_mode)
            ...
            for line in generate_file_lines(f):
                if not line.isspace():
                    ...
                    if re.match(r'</top>', line):
                        word = list(set(words))
                        content = False
                        q_terms = reduction(words, idf_lexicon,stem_lexicon, stem_dict, word, float(threshold),args.sort_mode)
                        if args.retrieval_mode == 'bm25':
